...
pub fn async_traversal_base(
    ...
    /* Parse the html text */
    let (tree, _traversal): (Arc<Element>, _) = parser(html_text).expect("Failed to parse HTML");
...
pub struct TraversalResult {
    pub matched_elements: Vec<Arc<Element>>,
    pub traversal_order: Vec<Arc<Element>>,
    pub paths_to_matches: Vec<Vec<Arc<Element>>>,
}
...
fn traverse_and_collect(
    ...
    current_path: &mut Vec<Arc<Element>>,
    ...
    current_path.push(Arc::clone(element));
    result.traversal_order.push(Arc::clone(element));

    let elem_ref = element.as_ref();

    for filter_list in filters {
        for filter in filter_list {
            if filter.selector.match_node(elem_ref, 0, elem_ref) {
                result.matched_elements.push(Arc::clone(element));
                result.paths_to_matches.push(current_path.clone());
                break;
            }
        }
    }
    ...
    for child in &element.children {
        if let Node::Element(c) = child {
            traverse_and_collect(c, filters, result, current_path, visited);
        }
    }
...
fn sync_traverse(html_text: &str, css_query: &str, use_dfs: bool) -> Option<TraversalResult> {
    let (tree, tokenizer_traversal) = parser(html_text).ok()?;
    ...
    let mut result = TraversalResult {
        ...
        paths_to_matches: Vec::new(),
    };

    let mut current_path: Vec<Arc<Element>> = Vec::new();
...
pub fn async_dfs(
    ...
    css_query: &str,
    _thread_num: usize,
) -> Option<Vec<Arc<Element>>> {
    let result = sync_traverse(html_text, css_query, true);
    result.map(|r| r.matched_elements)
}
...
pub fn async_bfs(
    ...
    let result = sync_traverse(html_text, css_query, false);
    result.map(|r| r.matched_elements)
}

pub fn get_traversal_result(
    html_text: &str,
    css_query: &str,
    use_dfs: bool,
) -> Option<TraversalResult> {
    sync_traverse(html_text, css_query, use_dfs)
}

pub fn get_traversal_order(html_text: &str) -> Option<Vec<(String, usize)>> {
    let (_tree, traversal) = parser(html_text).ok()?;
    let order: Vec<(String, usize)> = traversal
        .steps
        .iter()
        .filter_map(|s| s.tag.clone().map(|t| (t, s.position)))
        .collect();
    Some(order)
}
