use crate::css_selector::{
    AttributeFilter, CssSelectorParser, NodeFilter, PseudoFilter, SelectorUnit,
};
use crate::tokenizer::{Element, Node};

pub trait SelectorUnitExt {
    fn matches_response_node(
        &self,
        tag: &str,
        class: &str,
        id: &str,
...
impl SelectorUnitExt for SelectorUnit {
    fn matches_response_node(
        &self,
        tag: &str,
        class: &str,
        ...
        if let Some(ref selector_ids) = self.ids {
            if !selector_ids.is_empty() && id.is_empty() {
                return false;
            }
        ...
        if let Some(ref selector_classes) = self.classes {
            let node_classes: Vec<&str> = class.split_whitespace().collect();
            for selector_class in selector_classes {
                if !node_classes.contains(&selector_class.as_str()) {
                    return false;
                }
            }
        }
        ...
        if let Some(ref attrs) = self.attributes {
            for attr_filter in attrs {
                let attr_value = attributes
                    .iter()
                    .find(|(k, _)| k == &attr_filter.name)
                    .map(|(_, v)| v);
                match (attr_value, attr_filter.operator.as_deref()) {
                    (Some(value), Some("=")) => {
                        if let Some(expected) = &attr_filter.value {
                            if value != expected {
                                return false;
                            }
                        }
                    ...
                    (Some(value), Some("~=")) => {
                        if let Some(expected) = &attr_filter.value {
                            if !value.split_whitespace().any(|w| w == expected) {
                                return false;
                            }
                        }
                    }
                    (Some(value), Some("|=")) => {
                        if let Some(expected) = &attr_filter.value {
                            if value != expected && !value.starts_with(&format!("{}-", expected)) {
                                return false;
                            }
                        }
                    }
