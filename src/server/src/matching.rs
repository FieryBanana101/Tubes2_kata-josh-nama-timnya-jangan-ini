...
impl SelectorUnit {
    ...
    */
    fn match_pseudo_filter(&self, element: &Element, node_child_idx: usize, parent: &Element, filter: &PseudoFilter) -> bool {
        ...
        match filter.name.to_ascii_lowercase().as_str() {
            ...
            "first-of-type" => {
                let first_idx = parent.children.iter()
                    .enumerate()
            ...
            "last-of-type" => {
                let last_idx = parent.children.iter()
                    .enumerate()
                    .rev() 
                    .filter_map(|(idx, node)| match node {
                        Node::Element(e) => Some((idx, e)),
            ...
            "only-child" => {
                let element_count = parent.children.iter()
                    .filter(|node| {
                        if let Node::Element(_) = node { true }
                        else { false }
                    })
                    .count();
            ...
            "required" => {
                match element.tag.as_str() {
                    "input" | "textarea" | "select" => return element.attributes.contains_key("required"),
                    _ => return false,
                }
            },
            ...
            "read-write" => {
                ...
                match element.attributes.get("contenteditable").map(|value| value.as_str()) {
